...
//		判断list是否有,有,就做功能,没有就不做处理
		if(options.list != undefined && options.list.length > 0){
			options.list.eq(0).css({opacity:"1"})
...
//				根据鼠标点击的当前项的索引,添加当前项的样式
				options.list.eq($(this).index()).css({opacity:"0.5"}).siblings().css({opacity:"1"})
			})
...
//		左右按钮的move，根据正负号决定方向
		that.LOCAL.btnMove = function(type){
			...
			if(that.LOCAL.listOnoff){
				options.list.eq(that.LOCAL.index).css({opacity:"0.5"}).siblings().css({opacity:"1"})
			}
		}
