...
function magnifier(fatherName, MainWidth, MainHeight, img_src, SelectWidth, SelectHeight, BigBoxWidth) {
	var bei = (BigBoxWidth / SelectWidth); /** * 创建主图盒子,添加主图 */
	...
	$(".magnifierMainBox").mouseenter(function() {
		...
		var _left = e.clientX + $("html").scrollLeft() - $(".magnifierMainBox").offset().left - SelectWidth / 2;
		var _top = e.clientY + $("html").scrollTop() - $(".magnifierMainBox").offset().top - SelectHeight / 2;
		if(_left < 0) {
			_left = 0;
		}
